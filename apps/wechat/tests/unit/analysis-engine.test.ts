...
describe('analysis engine routing', () => {
  it('defaults fast to direct AI, standard to exact calibration, and precision to candidate-guided AI', () => {
    expect(defaultAnalysisEngineForMode('fast')).toBe('ai_direct')
    expect(defaultAnalysisEngineForMode('standard')).toBe('ai_then_db_exact')
  ...
  it('upgrades legacy stored choices without reintroducing the old forced DB route', () => {
    ...
    expect(normalizeAnalysisEngine('db_first', 'standard')).toBe('ai_then_db_exact')
