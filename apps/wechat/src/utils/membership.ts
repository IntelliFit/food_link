...
export const REWARD_LEVELS: RewardLevelMeta[] = [
...
export function getRewardLevelMeta(points: number): RewardLevelMeta {
  const normalized = Math.max(Number(points || 0), 0)
  return REWARD_LEVELS.find(level =>
    level.max == null
      ? normalized >= level.min
      : (normalized >= level.min && normalized < level.max)
  ) || REWARD_LEVELS[0]
}

export function getRewardLevelProgress(points: number, meta: RewardLevelMeta): number {
  const normalized = Math.max(Number(points || 0), 0)
...
export function formatRewardLevelRange(points: number, meta: RewardLevelMeta): string {
  const normalized = Math.max(Number(points || 0), 0)
