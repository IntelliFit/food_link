...
interface PetChatContentProps {
  embedded?: boolean
  active?: boolean
  starterQuestion?: string
  starterRequest?: number
  onMinimize?: () => void
}

export function PetChatContent({ embedded = false, active = true, starterQuestion, starterRequest, onMinimize }: PetChatContentProps) {
  ...
  const historyRequestBusyRef = useRef(false)
  ...
  const refreshConversation = useCallback(() => {
    if (!embedded) applyThemeNavigationBar(scheme)
    void Promise.all([
      getStatsSummary('week').then((data) => { if (mountedRef.current) setSummary(data) }).catch(() => null),
      getPetSummary().then((data) => { if (mountedRef.current) setPetSummary(data) }).catch(() => null),
    ])
    if (historyLoadedRef.current || restoringRef.current) return
    restoringRef.current = true
    setHistoryError('')
    setInitializing(true)
    void Promise.all([
      (async () => {
        let history = await getLatestPetChatSession()
        const latestSessionID = history?.session?.id || history?.session?.ID
        // The session endpoint returns the newest messages for long conversations.
        if (embedded && latestSessionID) history = await getPetChatSession(latestSessionID)
        ...
        if (mountedRef.current) setHistoryError('暂时没能读取对话，点此重试')
      }),
    ]).finally(() => {
  ...
  useDidShow(() => {
    if (!embedded) refreshConversation()
  })
  ...
  useEffect(() => {
    if (embedded && active) refreshConversation()
  }, [active, embedded, refreshConversation])

  useEffect(() => {
    if (starterQuestion) setInput(starterQuestion)
  }, [starterQuestion, starterRequest])

  useEffect(() => {
    if (!embedded) Taro.setNavigationBarTitle({ title: `和${petName}聊聊` })
    setMessages((prev) => {
    ...
    const timer = setTimeout(() => {
      void estimatePetChat(question, questionRange(question, activeRange), enableThinking)
        ...
        .catch((error) => {
          ...
          setEstimateError(String((error as Error)?.message || '暂时无法估算积分，请稍后再试'))
        })
        .finally(() => {
  ...
  }, [active, activeRange, enableThinking, input, latestDietRecommendation])
  ...
  const startNewConversation = useCallback(() => {
    if (busyRef.current || restoringRef.current || historyRequestBusyRef.current) return
    setHistoryError('')
    historyLoadedRef.current = true
  ...
  const openHistoryPanel = useCallback(async () => {
    if (busyRef.current || restoringRef.current || historyRequestBusyRef.current) return
    historyRequestBusyRef.current = true
    setHistoryOpen(true)
    ...
    try {
      ...
      historyRequestBusyRef.current = false
      setHistoryLoading(false)
    }
  }, [])

  const openSession = useCallback(async (targetSessionID: string) => {
    if (!targetSessionID || busyRef.current || restoringRef.current || historyRequestBusyRef.current) return
    historyRequestBusyRef.current = true
    setHistoryLoading(true)
    try {
  ...
  const runAnalysis = useCallback(async (question: string, range: RangeMode) => {
    ...
    cancelStreamRef.current = streamGeneratePetChat(question, range, sessionID, !sessionID, {
      onStart: () => {
      ...
      onError: async (error) => {
        ...
        updateMessage(streamingMessageID, () => ({
          ...
          role: 'pet',
          kind: 'local',
          text: '本次回复失败，问题已保留在输入框，可以稍后手动重试。',
          actions: ['换最近 7 天', '稍后再试'],
        }))
        setInput(question)
        finish()
        if (activeRef.current) await showUnifiedApiError(error, `${petName}分析失败`)
      },
    }, enableThinking)
  ...
  const handleSend = useCallback(() => {
    ...
    if (!text || restoringRef.current || historyRequestBusyRef.current || historyError || estimateError || busy || busyRef.current || estimatingCredits || estimatedCredits === null) return
    setInput('')
    const range = questionRange(text, activeRange)
    void runAnalysis(text, range)
  }, [activeRange, busy, estimateError, estimatedCredits, estimatingCredits, historyError, input, runAnalysis])

  const canSend = Boolean(input.trim()) && !initializing && !historyLoading && !historyError && !estimateError && !busy && !estimatingCredits && estimatedCredits !== null
  ...
  return (
    <View className={`pet-chat-page ${embedded ? 'pet-chat-page--embedded' : ''} ${scheme === 'dark' ? 'pet-chat-page--dark' : ''}`}>
      ...
      <ScrollView className='pet-chat-scroll' scrollY enhanced showScrollbar={false} scrollIntoView={latestMessageID}>
        ...
        {initializing ? <View className='pet-chat-thinking-bubble' aria-label='正在读取对话'><View className='pet-chat-thinking-dot' /><View className='pet-chat-thinking-dot' /><View className='pet-chat-thinking-dot' /></View> : null}
      ...
      <View className='pet-chat-bottom-dock'>
        ...
        {estimateError ? <Text className='pet-chat-estimate-error'>{estimateError}</Text> : null}
      </View>
