...
create index if not exists idx_precision_sessions_user_created_at
  on public.precision_sessions using btree (user_id, created_at desc);

create index if not exists idx_precision_sessions_user_status
  on public.precision_sessions using btree (user_id, status);
...
create index if not exists idx_precision_session_rounds_session_round
  on public.precision_session_rounds using btree (session_id, round_index asc, created_at asc);
...
create index if not exists idx_precision_item_estimates_session_round
  on public.precision_item_estimates using btree (session_id, round_index, item_index asc);

create index if not exists idx_precision_item_estimates_source_task_id
  on public.precision_item_estimates using btree (source_task_id);
