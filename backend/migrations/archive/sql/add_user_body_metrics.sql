...
alter table public.user_weight_records
  add column if not exists client_record_id text;

alter table public.user_weight_records
  drop constraint if exists user_weight_records_user_date_unique;

create index if not exists idx_user_weight_records_user_date
  on public.user_weight_records using btree (user_id, recorded_on);

create index if not exists idx_user_weight_records_user_created_at
  on public.user_weight_records using btree (user_id, created_at desc);
...
create index if not exists idx_user_water_logs_user_date
  on public.user_water_logs using btree (user_id, recorded_on);

create index if not exists idx_user_water_logs_user_recorded_at
  on public.user_water_logs using btree (user_id, recorded_at desc);
