...
create index if not exists idx_food_expiry_items_user_id
  on public.food_expiry_items using btree (user_id) tablespace pg_default;

create index if not exists idx_food_expiry_items_user_status
  on public.food_expiry_items using btree (user_id, status) tablespace pg_default;

create index if not exists idx_food_expiry_items_user_expire_date
  on public.food_expiry_items using btree (user_id, expire_date) tablespace pg_default;
