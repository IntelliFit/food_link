...
CREATE TABLE IF NOT EXISTS public.comment_tasks (
...
-- 兼容老库：若表已存在旧结构，需要补 extra 字段
ALTER TABLE public.comment_tasks
  ADD COLUMN IF NOT EXISTS extra jsonb NULL;

-- 索引：按用户查询
CREATE INDEX IF NOT EXISTS idx_comment_tasks_user_id ON public.comment_tasks USING btree (user_id) TABLESPACE pg_default;

-- 索引：按状态和类型查询（Worker 抢占任务）
CREATE INDEX IF NOT EXISTS idx_comment_tasks_status_type ON public.comment_tasks USING btree (status, comment_type) TABLESPACE pg_default;

-- 索引：按创建时间排序
CREATE INDEX IF NOT EXISTS idx_comment_tasks_created_at ON public.comment_tasks USING btree (created_at) TABLESPACE pg_default;
...
COMMENT ON COLUMN public.comment_tasks.extra IS '评论补充上下文，如 parent_comment_id / reply_to_user_id';
