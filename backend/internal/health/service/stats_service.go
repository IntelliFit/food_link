		...
		if apiKey == "" {
			...
			if err != nil {
				...
				for text := range textChan {
					cleanText := sanitizePetChatStreamText(text)
					if cleanText == "" {
						continue
					}
					if !firstChunkLogged {
					...
					fullAnswer.WriteString(cleanText)
					chunkChan <- PetChatStreamChunk{Type: "chunk", Text: cleanText}
				}
...
func sanitizePetChatStreamText(content string) string {
