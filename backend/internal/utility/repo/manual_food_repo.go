...
func manualFoodResultFromPackaged(item fooddomain.PackagedFood, score float64) domain.ManualFoodResult {
	...
	defaultScale := defaultWeight / 100
	if defaultScale <= 0 {
		defaultScale = 1
	}
	result := domain.ManualFoodResult{
		...
		DefaultWeightGrams: defaultWeight,
		TotalCalories:      item.KcalPer100g * defaultScale,
		TotalProtein:       item.ProteinPer100g * defaultScale,
		TotalCarbs:         item.CarbsPer100g * defaultScale,
		TotalFat:           item.FatPer100g * defaultScale,
		...
		ExtraNutrients: &domain.ManualFoodNutrients{
			Fiber:          item.FiberPer100g * defaultScale,
			Sugar:          item.SugarPer100g * defaultScale,
			SaturatedFat:   item.SaturatedFatPer100g * defaultScale,
			CholesterolMg:  item.CholesterolMgPer100g * defaultScale,
			SodiumMg:       item.SodiumMgPer100g * defaultScale,
			PotassiumMg:    item.PotassiumMgPer100g * defaultScale,
			CalciumMg:      item.CalciumMgPer100g * defaultScale,
			IronMg:         item.IronMgPer100g * defaultScale,
			MagnesiumMg:    item.MagnesiumMgPer100g * defaultScale,
			ZincMg:         item.ZincMgPer100g * defaultScale,
			VitaminARaeMcg: item.VitaminARaeMcgPer100g * defaultScale,
			VitaminCMg:     item.VitaminCMgPer100g * defaultScale,
			VitaminDMcg:    item.VitaminDMcgPer100g * defaultScale,
			VitaminEMg:     item.VitaminEMgPer100g * defaultScale,
			VitaminKMcg:    item.VitaminKMcgPer100g * defaultScale,
			ThiaminMg:      item.ThiaminMgPer100g * defaultScale,
			RiboflavinMg:   item.RiboflavinMgPer100g * defaultScale,
			NiacinMg:       item.NiacinMgPer100g * defaultScale,
			VitaminB6Mg:    item.VitaminB6MgPer100g * defaultScale,
			FolateMcg:      item.FolateMcgPer100g * defaultScale,
			VitaminB12Mcg:  item.VitaminB12McgPer100g * defaultScale,
		},
		ImagePath:           imagePath,
		ImagePaths:          imagePaths,
