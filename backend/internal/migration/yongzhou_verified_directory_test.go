...
func TestYongzhouVerifiedDiningSeed(t *testing.T) {
	spec := yongzhouVerifiedDiningSpec
	spec.SeedPath = "../../data/yongzhou_verified_dining_seed_20260820.json"
	...
	assert.Equal(t, 3, totalCanteens)
	assert.Equal(t, 4, totalFloorRelations)
	assert.Equal(t, 7, totalSources)
}
