...
func TestShenyangVerifiedDiningSeed(t *testing.T) {
	spec := shenyangVerifiedDiningSpec
	spec.SeedPath = "../../data/shenyang_verified_dining_seed_20260816.json"
	batch, err := loadVerifiedDiningBatch(spec)
	require.NoError(t, err)
	require.Len(t, batch.Schools, 8)
	totalCampuses := 0
	totalCanteens := 0
	...
	assert.Equal(t, 8, totalCampuses)
	assert.Equal(t, 12, totalCanteens)
	assert.Equal(t, 25, totalFloorRelations)
	assert.Equal(t, 22, totalSources)
}
