...
func TestXinxiangVerifiedDiningSeed(t *testing.T) {
	spec := xinxiangVerifiedDiningSpec
	spec.SeedPath = "../../data/xinxiang_verified_dining_seed_20260816.json"
	batch, err := loadVerifiedDiningBatch(spec)
	require.NoError(t, err)
	require.Len(t, batch.Schools, 4)
	totalCampuses := 0
	totalCanteens := 0
	...
	assert.Equal(t, 5, totalCampuses)
	assert.Equal(t, 8, totalCanteens)
	assert.Equal(t, 14, totalFloorRelations)
	assert.Equal(t, 16, totalSources)
}
