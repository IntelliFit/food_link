...
func TestJinzhouVerifiedDiningSeed(t *testing.T) {
	spec := jinzhouVerifiedDiningSpec
	spec.SeedPath = "../../data/jinzhou_verified_dining_seed_20260816.json"
	...
	assert.Equal(t, 4, totalCanteens)
	assert.Equal(t, 6, totalFloorRelations)
	assert.Equal(t, 11, totalSources)
}
