...
func ensureConstraints(ctx context.Context, db *gorm.DB) error {
	for _, sql := range []string{
		...
		dropAndAddCheck("feed_likes", "feed_likes_target_type_check", `target_type = ANY (ARRAY['food_record'::text,'exercise_log'::text])`),
		dropAndAddCheck("feed_comments", "feed_comments_target_type_check", `target_type = ANY (ARRAY['food_record'::text,'exercise_log'::text])`),
		dropAndAddCheck("feed_interaction_notifications", "feed_interaction_notifications_target_type_check", `target_type = ANY (ARRAY['food_record'::text,'exercise_log'::text])`),
