...
func TestChenzhouVerifiedDiningSeed(t *testing.T) {
	spec := chenzhouVerifiedDiningSpec
	spec.SeedPath = "../../data/chenzhou_verified_dining_seed_20260820.json"
	...
	assert.Equal(t, 1, totalCanteens)
	assert.Equal(t, 1, totalFloorRelations)
	assert.Equal(t, 3, totalSources)
}
