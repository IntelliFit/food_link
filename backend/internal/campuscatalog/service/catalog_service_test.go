...
func TestPublishItemRetriesImageTimeoutWithoutDroppingImageEvidence(t *testing.T) {
	price := 12.0
	repo := &fakeCatalogRepo{existingItem: &domain.CatalogItem{
	...
	submitter := &fakeCatalogAnalyzeSubmitter{taskID: "task-image-retry"}
	svc := NewCatalogService(repo, nil)
	svc.ConfigureAnalysis(submitter, fakeCatalogAnalysisUserResolver{userID: "system-user-1"})
	...
	require.Equal(t, "image", submitter.input.SourceType)
	require.Equal(t, []string{"campus-food/roll.jpg"}, submitter.input.ImageURLs)
	require.Empty(t, submitter.textInput.SourceType)
}
