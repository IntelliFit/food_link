...
func TestIsOfficialCandidate(t *testing.T) {
	...
	assert.False(t, isOfficialCandidate("file://hq.example.edu.cn/private.txt"))
}
...
func TestSaveJSONAtomicallyReplacesExistingState(t *testing.T) {
	directory := t.TempDir()
	path := filepath.Join(directory, "crawl-state.json")
	require.NoError(t, os.WriteFile(path, []byte(`{"status":"old"}`), 0o644))

	require.NoError(t, saveJSON(path, map[string]any{
		"status":  "complete",
		"schools": []string{"school-1"},
	}))

	data, err := os.ReadFile(path)
	require.NoError(t, err)
	assert.JSONEq(t, `{"status":"complete","schools":["school-1"]}`, string(data))
	temporaryFiles, err := filepath.Glob(filepath.Join(directory, ".crawl-state.json.*.tmp"))
	require.NoError(t, err)
