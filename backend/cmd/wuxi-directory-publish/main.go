...
func main() {
	...
	if *dryRun {
		result, err := migration.InspectWuxiVerifiedDiningDirectory(ctx, db, schema)
		if err != nil {
			log.Fatalf("检查无锡高校目录失败: %v", err)
		}
		log.Printf("无锡高校目录 dry-run: schools=%d campuses=%d canteens=%d existing_campuses=%d new_campuses=%d existing_canteens=%d new_canteens=%d", result.Schools, result.Campuses, result.Canteens, result.ExistingCampuses, result.NewCampuses, result.ExistingCanteens, result.NewCanteens)
		return
	}
	if err := migration.PublishWuxiVerifiedDiningDirectory(ctx, db, schema); err != nil {
		log.Fatalf("发布无锡高校目录失败: %v", err)
	}
	log.Printf("无锡高校目录发布完成: target=%s", target)
}
