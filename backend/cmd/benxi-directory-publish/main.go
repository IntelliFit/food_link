...
func main() {
	...
	if *dryRun {
		result, err := migration.InspectBenxiVerifiedDiningDirectory(ctx, db, schema)
		if err != nil {
			log.Fatalf("检查本溪高校目录失败: %v", err)
		}
		log.Printf("本溪高校目录 dry-run: schools=%d campuses=%d canteens=%d existing_campuses=%d new_campuses=%d existing_canteens=%d new_canteens=%d", result.Schools, result.Campuses, result.Canteens, result.ExistingCampuses, result.NewCampuses, result.ExistingCanteens, result.NewCanteens)
		return
	}
	if err := migration.PublishBenxiVerifiedDiningDirectory(ctx, db, schema); err != nil {
		log.Fatalf("发布本溪高校目录失败: %v", err)
	}
	log.Printf("本溪高校目录发布完成: target=%s", target)
}
