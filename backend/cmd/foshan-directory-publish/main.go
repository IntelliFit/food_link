...
func main() {
	...
	if *dryRun {
		result, err := migration.InspectFoshanVerifiedDiningDirectory(ctx, db, schema)
		if err != nil {
			log.Fatalf("检查佛山高校目录失败: %v", err)
		}
		log.Printf("佛山高校目录 dry-run: schools=%d campuses=%d canteens=%d existing_campuses=%d new_campuses=%d existing_canteens=%d new_canteens=%d", result.Schools, result.Campuses, result.Canteens, result.ExistingCampuses, result.NewCampuses, result.ExistingCanteens, result.NewCanteens)
		return
	}
	if err := migration.PublishFoshanVerifiedDiningDirectory(ctx, db, schema); err != nil {
		log.Fatalf("发布佛山高校目录失败: %v", err)
	}
	log.Printf("佛山高校目录发布完成: target=%s", target)
}
