...
func main() {
	...
	if *dryRun {
		result, err := migration.InspectXingtaiVerifiedDiningDirectory(ctx, db, schema)
		if err != nil {
			log.Fatalf("检查邢台高校目录失败: %v", err)
		}
		log.Printf("邢台高校目录 dry-run: schools=%d campuses=%d canteens=%d existing_campuses=%d new_campuses=%d existing_canteens=%d new_canteens=%d", result.Schools, result.Campuses, result.Canteens, result.ExistingCampuses, result.NewCampuses, result.ExistingCanteens, result.NewCanteens)
		return
	}
	if err := migration.PublishXingtaiVerifiedDiningDirectory(ctx, db, schema); err != nil {
		log.Fatalf("发布邢台高校目录失败: %v", err)
	}
	log.Printf("邢台高校目录发布完成: target=%s", target)
}
