...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum pages to fetch for testing; default is unlimited.",
    )
    parser.add_argument(
        "--rate-sleep-seconds",
        type=float,
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Fetch and parse only; do not write into the nutrition library.",
    )
    parser.add_argument(
        "--update-existing",
        action="store_true",
        help="Update existing rows when normalized_name already exists.",
    )
    parser.add_argument(
        "--json-output",
        default="",
...
def main() -> int:
    ...
    if args.source_mode in {"api", "auto"} and not args.api_key:
        print("Missing USDA API key. Set USDA_FDC_API_KEY or pass --api-key.", file=sys.stderr)
        return 2
