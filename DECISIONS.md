...
- `2026-04-21`（实现口径）：三档 × 三周期会员已在本代码库落地一期。正式实现口径：
  ...
  - 积分状态以后端 `/api/membership/me` 为唯一真源：新用户 3 天试用（按 `weapp_user.created_at` 判定，试用期每日 8 积分）、付费用户按自己套餐的 `daily_credits`；积分消耗口径固定为「食物分析 2 / 运动记录 1」；`credits_reset_at` 始终按用户本地中国时区次日 0 点返回；积分当日有效、次日清零、永不累计
  - 前端不再依赖旧的「今日拍照 x/y」口径，会员页与我的页统一改为「今日积分 x/y」与 `trial_active / trial_expires_at` 显示
  - 一期仅做「展示 + 积分试算」，不在分析提交链路做扣减/拦截；扣减 enforcement 与邀请/分享奖励、微信自动续费、`user_membership_payments.pending` 清理均推迟到后续阶段
