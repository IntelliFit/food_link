...
- `2026-04-22`: `health-reports` 的私有属性属于固定安全口径，不再通过环境变量开关控制；后端代码中直接写死为私有桶，避免误配置把体检报告暴露到公网。

- `2026-04-22`: 所有对象 key 保持与 Supabase 时代一致，不在迁移时引入按年月日重新分目录的重排策略，优先保证迁移稳定性与历史引用兼容。

- `2026-04-22`: 历史对象不迁移重排，但从 COS 切换完成后，所有新上传对象统一按中国自然日分层。具体口径：`food-images -> YYYY/MM/DD/uuid`；`user-avatars` 与 `health-reports` 保留用户维度，使用 `user_id/YYYY/MM/DD/uuid`。

- `2026-04-22`: 后端所有与对象存储相关的 URL 解析必须统一兼容三种输入：旧 Supabase Storage URL、新腾讯云 CDN URL、原始对象 key；删除、压缩、孤儿清理、OCR 读取不得各自维护独立规则。

- `2026-04-22`: 数据库存量若仍保存完整存储 URL，后端读取时统一按“可信来源 URL -> 提取 key -> 按当前 `.env` 重建 CDN/签名地址”处理；仅认可当前项目的 Supabase/COS/CDN 来源，公开桶对未知外链最多原样兼容，私有桶不得对未知外链生成可信访问地址。

- `2026-04-22`: 历史数据清洗脚本只允许直连 `POSTGRESQL_*` 指向的目标 PostgreSQL，默认 dry-run，显式 `--apply` 才执行；脚本需支持重复运行，并在目标 host 疑似 Supabase 或与源库同 host 时直接拒绝执行。
