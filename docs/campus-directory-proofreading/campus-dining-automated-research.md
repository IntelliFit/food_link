...
脚本先搜索学校官网域名，再使用站内限定查询寻找食堂、楼层和窗口页面；高校官网、政府网站、阳光高考和可访问的微信公众号页面会进入候选。`-concurrency` 控制学校 worker 数（1–8，默认 2）；网络请求仍由线程安全的全局限速器控制启动间隔，既能重叠慢站点等待，又不会让每个 worker 各自无上限请求。抓取结果由主协程逐校写入同一个断点 JSON，避免并发写坏文件。
