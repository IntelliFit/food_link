...
import { getStatsSummary, generateStatsInsight, saveStatsInsight, getBodyMetricsSummary, type StatsSummary, type BodyMetricWeightEntry, type BodyMetricWaterDay } from '../../utils/api'
import { WATER_GOAL_DEFAULT } from '../index/utils/constants'
...
// 从本地存储获取体重/喝水数据（作为云端数据的备选）
function getStoredBodyMetrics() {
  try {
...
export default function StatsPage() {
  ...
  const fetchStats = useCallback(async (r: 'week' | 'month') => {
    ...
    try {
      ...
      // 获取本地存储的数据作为备选
      const storedMetrics = getStoredBodyMetrics()
      
      // 检查云端是否有数据
      const hasCloudWeight = bodyMetricsRes?.weight_entries && bodyMetricsRes.weight_entries.length > 0
      const hasCloudWater = bodyMetricsRes?.water_daily && bodyMetricsRes.water_daily.some((d: any) => d.total > 0)
      ...
      const storedWaterDaily = Object.values(storedMetrics.waterByDate).map((w: any) => ({
        date: w.date,
        total: w.total,
        logs: w.logs || []
      }))
      ...
      // 使用云端数据（如果有），否则使用本地数据
      const weightEntries = hasCloudWeight ? bodyMetricsRes!.weight_entries : storedWeightEntries
      const waterDaily = hasCloudWater ? bodyMetricsRes!.water_daily : storedWaterDaily
      
      // 计算统计数据
      const totalWaterMl = waterDaily.reduce((sum: number, d: any) => sum + (d.total || 0), 0)
      const recordedDays = waterDaily.filter((d: any) => (d.total || 0) > 0).length
      ...
      statsRes.body_metrics = {
        ...
        water_goal_ml: bodyMetricsRes?.water_goal_ml || storedMetrics.waterGoalMl || WATER_GOAL_DEFAULT,
          ...
          <View className='card-header'>
            ...
            <View className='card-header-copy'>
              ...
              <Text className='card-subtitle'>
                体重记录:{weightTrend.length}条 喝水记录:{waterDaily.length}天
              </Text>
            </View>
          </View>
