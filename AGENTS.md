...
- 本项目所有日常代码和文档改动默认直接在 `dev` 分支进行，并在开始修改前确认当前分支为 `dev`。
...
- 主包必须控制在 1464KB 以内（1,499,136 字节，严格小于十进制 1.5MB），1400KB 起预警。
- 每个独立分包必须严格小于或等于微信 2MB 硬限制，并执行项目既有的 2000KB 分包预算。
- 上传或交付调试前必须实际测量；任何一个包超过对应预算都不得继续上传。
