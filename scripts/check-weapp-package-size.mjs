...
const DEFAULT_BUDGET_KB = 2000
const DEFAULT_WARN_KB = 1950
const DEFAULT_MAIN_BUDGET_KB = 1464
const DEFAULT_MAIN_WARN_KB = 1400
...
function parseArgs(argv) {
  const options = {
    ...
    mainWarnKb: DEFAULT_MAIN_WARN_KB,
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (arg === '--dist' && next) {
      ...
      i += 1
    } else if (arg === '--main-budget-kb' && next) {
      options.mainBudgetKb = Number(next)
      i += 1
    } else if (arg === '--main-warn-kb' && next) {
      options.mainWarnKb = Number(next)
      i += 1
...
Options:
  ...
  --main-budget-kb <num> Main package enforced budget, default: ${DEFAULT_MAIN_BUDGET_KB}
  --main-warn-kb <num>   Main package warning threshold, default: ${DEFAULT_MAIN_WARN_KB}
...
function thresholdsFor(report, options) {
  if (report.name === 'main') {
    return {
      maxKb: options.maxKb,
      budgetKb: options.mainBudgetKb,
      warnKb: options.mainWarnKb,
    }
  }
  return {
    maxKb: options.maxKb,
    budgetKb: options.budgetKb,
    warnKb: options.warnKb,
  }
}
...
function main() {
  ...
  if (options.mainWarnKb > options.mainBudgetKb || options.mainBudgetKb > options.maxKb) {
    throw new Error('主包体积阈值必须满足 main-warn-kb <= main-budget-kb <= max-kb')
  }
  ...
  console.log(`[weapp:size] main package enforced budget=${options.mainBudgetKb}KB (<1.5MB decimal), warning=${options.mainWarnKb}KB`)

  for (const report of reports) {
    const thresholds = thresholdsFor(report, options)
    const status = statusFor(report.bytes, thresholds.maxKb, thresholds.budgetKb, thresholds.warnKb)
    const line = `${status.padEnd(4)} ${report.name.padEnd(24)} ${formatKb(report.bytes).padStart(10)}  ${report.files.length} files`
    console.log(line)
    if (status === 'FAIL') {
      failures.push(`${report.name} ${formatKb(report.bytes)} > ${thresholds.maxKb}KB`)
    }
    if (status === 'BUDGET') {
      failures.push(`${report.name} ${formatKb(report.bytes)} > enforced budget ${thresholds.budgetKb}KB`)
    }
