...
- 校园卡片通过 `food-card--campus` 独立缩至 144rpx 图片、14rpx 主内容内边距、12rpx 卡间距和 10rpx × 14rpx 作者栏内边距；普通公共食物卡保持上一版 176rpx 图片、18rpx 内距和 16rpx 间距。
- “食探官方”只显示文字，不渲染头像或空头像占位；普通用户仍保留头像与昵称。
- 运行态读取到 50 张校园卡片、50 行位置、50 行营养摘要、50 个官方文字署名、0 个作者头像；最终首卡约 371 × 115 CSS px，图片约 74 × 74 CSS px，相比上一版 138px 卡高再减少约 17%。
- 运行态切换“校园食堂”标签成功且仍显示 50 张卡片，错误事件为 0；隔离暂存快照的 TypeScript、全量 ESLint、33 套/111 条 Jest、Sass 与 `git diff --check` 均通过。

## Comparison history

1. 第一轮将图片由 220rpx 缩至 176rpx，卡片运行态高度从原图约 165px 级别收紧到 138px。
2. 用户要求在删除详情层后继续缩小；第二轮仅对校园卡应用 144rpx 图片、14rpx 主内距和 12rpx 卡间距，运行态卡高曾测得 115px，菜名、位置和营养仍保持三行清晰结构；普通卡不继承第二轮尺寸。

## Blocking issue

- 微信开发者工具自动化可以读取页面、节点、尺寸并完成标签交互，但 `App.captureScreenshot` 持续超时。因此无法生成最终实现截图，也无法按 Product Design 要求把源图与实现图合成同视口并排视觉对比；本轮视觉截图 QA 继续标记为 blocked。
